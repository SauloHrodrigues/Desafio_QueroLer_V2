...
class LivroServeceImplTest {
    ...
    @Test
    @DisplayName("Deve buscar um livro pelo autor com sucesso")
    void deveBuscarUmLivroPorAutor() {
        Livro livro = LivroFixture.entity();
        String autor = livro.getAutores().get(0).getNome();
        Pageable pageable = PageRequest.of(0, 10);
        List<Livro> livros = List.of(livro, livro);
        Page<Livro> pageLivros = new PageImpl<>(livros, pageable, livros.size());
        LivroResponse response = LivroFixture.response();
        ...
        Page<LivroResponse> resposta = service.buscarAutor(autor, pageable);

        assertNotNull(resposta);
        assertEquals(livros.size(), resposta.getContent().size());
        verify(mapper, times(2)).toResponse(livro);
    }
    ...
    void deveBuscarUmLivroPeloTitulo() {
        ...
        Page<LivroResponse> resposta = service.buscarPorTitulo(titulo, pageable);
    ...
    @Test
    @DisplayName("Deve buscar um livro pela editora com sucesso")
    void deveBuscarUmLivroPelaEditora() {
        Livro livro = LivroFixture.entity();
        ...
        Page<LivroResponse> resposta = service.buscarPorEditora(editora, pageable);
